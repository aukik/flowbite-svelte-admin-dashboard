...
const fetchAllUserData = async (token) => {
  try {
	...
	const response = await axios.get(`${BASE_URL}/admin/allcompetitionData`, {
	  headers: {
...
<main class="relative h-full w-full overflow-y-auto bg-white dark:bg-gray-800">
	<div class="p-4">
		...
		<Toolbar embedded class="w-full py-4 text-gray-500  dark:text-gray-400">
			...
			<div slot="end" class="flex items-center space-x-2">
				...
				>
					<PlusOutline size="sm" />Add a Competition
				</Button>
	...
	<Table>
		<TableHead class="border-y border-gray-200 bg-gray-100 dark:border-gray-700">
			...
			{#each ['Name', 'Motto','Club', 'People joined','Competition Type','Location','Actions'] as title}
		...
		<TableBody>
			{#each userData as user}
				<TableBodyRow class="text-base">
					...
					<TableBodyCell class="p-4">{user.competition_type||"N/A"}</TableBodyCell>
