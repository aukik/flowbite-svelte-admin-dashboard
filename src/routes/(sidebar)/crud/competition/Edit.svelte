...
let event_type_label = data?.event_type;
  ...
  async function handleSubmit() {
    ...
    console.log(data.id);
	console.log(token);
	let eventUpdate_api = apiUrl + '/admin/competitionUpdate/';
		...
		<form on:submit={handleSubmit} use:init>
			<div class="grid grid-cols-6 gap-6">
                ...
                <Label class="col-span-6 space-y-2 sm:col-span-3">
					...
					<div class="pt-5">
						<Button >{data.competition_type}<ChevronDownOutline class="w-6 h-6 ms-2 text-white dark:text-white" /></Button>
						<Dropdown>
